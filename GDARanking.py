...
#-----------------------------------------------#
# 1. Explain nodes Phat V P										  #
# 2. Intersect with LPhat or Phat that where LP #
 ##############################################
...
def isCandidable(lbl, pred):
	# 				LPhat    or 	Phat 		 and 		LP
	return (pred == 1) or (pred == 0 and lbl == 1)

def save_timings(times, disease_id, method):
	fname = 'Scalability/'+disease_id+'_'+method+'.txt'
...
def predict_candidate_genes_gnn_explainer(model, dataset, predictions, disease_Id, explanation_nodes_ratio=1, masks_for_seed=10, G=None, num_pos='all'):
	...
	i = 0
	for node in G:
			if labels[i] == 0 and predictions[i] == 0: # if the nodes is P and also predicted as P
	...
	# List to store computation times: at position i is store the time required to explain node i
	times = []

	# Get explanations of all the positive genes
	nodes_explained = 0
	for node in tqdm(nodes_with_idxs, desc=str(os.getpid())+' - Explaining'):
		# start timer
		...
		for i in range(len(indices)):
			...
			if isCandidable(labels[src], src_pred):
				if src_name not in candidates[node]:
						candidates[node][src_name] = values[i]
				else:
						candidates[node][src_name] += values[i]

			if isCandidable(labels[trgt], trgt_pred): # LP
	...
	save_timings(times, disease_Id, 'GNNExplainer')
...
def predict_candidate_genes_gnn_explainer_only(model, dataset, predictions, disease_Id, explanation_nodes_ratio=1, masks_for_seed=10, G=None, num_pos='all'):
	...
	for node in tqdm(nodes_with_idxs, desc=str(os.getpid())+' - Explaining'):
		...
		for i in range(len(indices)):
			...
			if isCandidable(predictions[src], src_pred): # LP # no needed here but unlabelled
					if src_name not in candidates[node]:
							candidates[node][src_name] = values[i]
					else:
							candidates[node][src_name] += values[i]

			if isCandidable(predictions[trgt], trgt_pred): # LP #no needed here but unlabelled
					if trgt_name not in candidates[node]:
							candidates[node][trgt_name] = values[i]
					else:
							candidates[node][trgt_name] += values[i]
	...
	save_timings(times, disease_Id, 'GNNExplaner_only')
...
def predict_candidate_genes_graphsvx(model, dataset, predictions, disease_Id, explanation_nodes_ratio=1, num_hops=1, G=None, num_pos="all", threshold = False):
	...
	for node in tqdm(nodes_with_idxs, desc=str(os.getpid())+' - Explaining'):
		...
		for i in range(len(important_nodes)):
			...
			if isCandidable(labels[src], src_pred): # LP
					if src_name not in candidates[node]:
	...
	save_timings(times, disease_Id, 'GraphSVX')
...
def predict_candidate_genes_graphsvx_only(model, dataset, predictions, disease_Id, explanation_nodes_ratio=1, num_hops=1, G=None, num_pos="all", threshold = True):
	...
	for node in tqdm(nodes_with_idxs, desc=str(os.getpid())+' - Explaining'):
		...
		for i in range(len(important_nodes)):
			...
			if isCandidable(predictions[src], src_pred): # here 1 is unlabelled. We look for candidates in the unlabelled set
				if src_name not in candidates[node]:
					candidates[node][src_name] = nodes_and_explanations[src]
				else:
					candidates[node][src_name] += nodes_and_explanations[src]
	...
	save_timings(times, disease_Id, 'GraphSVX_only')
...
def run_explanation(args):
	...
	for coalition_node_idx in ori_nodes_idxs:
		if coalition_node_idx != idx:
			...
			pred = predictions[coalition_node_idx]
	...
	return candidates, end_time

def predict_candidate_genes_subgraphx(model, dataset, predictions, disease_Id, explanation_nodes_ratio, num_hops, G, num_workers=1, num_classes=5):
	labels = dataset.y.to('cpu')
	...
	save_timings(times, disease_Id, 'SubgraphX' if num_classes == 5 else 'SubgraphX_only')
	return sorted_ranking
