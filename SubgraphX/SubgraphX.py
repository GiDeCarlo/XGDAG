...
def find_closest_node_result(results, max_nodes):
    """ return the highest reward tree_node with its subgraph is smaller than max_nodes """
    #My edit: check for type 
    result_type = type(results[0])
    if result_type is dict:
        results = sorted(results, key=lambda x: len(x["coalition"])) #MY EDIT, there was no if and only x.coalition
    else:
         results = sorted(results, key=lambda x: len(x.coalition))
...
def find_closest_node_result_fixed_nodes(results, max_nodes):
    """ return the highest reward tree_node with its subgraph is smaller than max_nodes """
    #My edit: check for type 
    result_type = type(results[0])
    if result_type is dict:
