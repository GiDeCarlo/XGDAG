...
def get_dataset_from_graph(path_to_graph, dataset_path, disease_id, mode = "multiclass", verbose=True, quartiles=False, from_diamond=False, seed = 42):
		if mode == 'binary':
			print("Quartiles division is not used in binary classification. Setting quartiles to False.")
			quartiles = False
		...
		path = dataset_path if not from_diamond else dataset_path + 'Diamond_dataset/'

		path_to_seed_genes = None

		if not from_diamond:
			path_to_seed_genes = path + "/curated_seed_genes/" + disease_id + '_seed_genes'
		else:
			path_to_seed_genes = path + '/seed_genes/' + disease_id + '_seed_genes_diamond'
		path_to_seed_genes += '.txt'
		...
		nedbit_scores = pd.read_csv(path + "niapu_rankings/" + disease_id + '_niapu_ranking.txt', sep=' ', header=None)
		...
		if not quartiles and mode == 'multiclass':
			...
			nedbit_scores["label"].replace(to_replace = 1, value = "P", inplace = True)
			nedbit_scores["label"].replace(to_replace = 2, value = "U", inplace = True)
			nedbit_scores["label"].replace(to_replace = 3, value = "U", inplace = True)
			nedbit_scores["label"].replace(to_replace = 4, value = "U", inplace = True)
			nedbit_scores["label"].replace(to_replace = 5, value = "U", inplace = True)
		...
		NUM_CLASSES = 5 if mode == 'multiclass' else 2
